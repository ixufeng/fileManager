	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '<form id="uploadForm" action="upload.do?path='+path+'" method="post" enctype="multipart/form-data">';
		            ...
		            html += '               <div id="upload_over" onclick="hideuploadBorder();">结束上传</div>';
		            html += '				<div class="btns">';
					...
					// 创建不带有拖动的html
					html += '<form id="uploadForm" action="upload.handle.php?path='+path+'" method="post" enctype="multipart/form-data">';
					html += '	<div class="upload_box">';
		            ...
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		            html += '                       <input type="text" name = "path" value = "'+$('#icon_home').attr('path')+'">';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						$("#uploadInf").append("正在上传...：" + response + "</p>");
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
